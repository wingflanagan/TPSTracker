...
var
  Hours: Double;
begin
  // Initialize Hours to 0 to handle cases where FCumulativeTime is 0
  Hours := 0;

  // Convert FCumulativeTime from seconds to hours
  if FCumulativeTime <> 0 then
    Hours := FCumulativeTime / 3600;

  // Return 0 if the tracker has not been started and has no accumulated time
  if (Hours = 0) and not FIsRunning then
    Result := 0
  else if Hours < FTimeResolution then
    // Return FTimeResolution only if the tracker is running and Hours is less than FTimeResolution
    Result := FTimeResolution
  else
    // Adjust the cumulative time to match the time resolution increments
